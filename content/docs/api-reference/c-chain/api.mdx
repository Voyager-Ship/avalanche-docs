...
<Callout title="Note">
Ethereum has its own notion of `networkID` and `chainID`. These have no relationship to Avalanche's view of networkID and chainID and are purely internal to the [C-Chain](/learn/primary-network#c-chain). On Mainnet, the C-Chain uses `1` and `43114` for these values. On the Fuji Testnet, it uses `1` and `43113` for these values. `networkID` and `chainID` can also be obtained using the `net_version` and `eth_chainId` methods.
</Callout>
...
On localhost, use `ws://`. When using the [Public API](/tooling/rpc-providers) or another
...
<Callout title="info">
For batched requests on the [public API node](/tooling/rpc-providers) , the maximum
...
documentation](/api-reference/standards/guides/txn-fees#c-chain-fees).
...
with the Admin API enabled, use [C-Chain config flag`--coreth-admin-api-enabled:true`](/nodes/chain-configs/c-chain#coreth-admin-api-enabled).
