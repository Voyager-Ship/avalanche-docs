...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'Playground_user_id_fkey'
    ) THEN
        ALTER TABLE "Playground" ADD CONSTRAINT "Playground_user_id_fkey" 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'PlaygroundFavorite_playground_id_fkey'
    ) THEN
        ALTER TABLE "PlaygroundFavorite" ADD CONSTRAINT "PlaygroundFavorite_playground_id_fkey" 
        FOREIGN KEY ("playground_id") REFERENCES "Playground"("id") ON DELETE CASCADE ON UPDATE CASCADE;
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'PlaygroundFavorite_user_id_fkey'
    ) THEN
        ALTER TABLE "PlaygroundFavorite" ADD CONSTRAINT "PlaygroundFavorite_user_id_fkey" 
        FOREIGN KEY ("user_id") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;
    END IF;
END $$;